...
class UI {
  ...
  button(id, x, y, w, h, label, opts) {
    ...
    if (!o.invisible) {
      this.panel(x, y, w, h, {
        color: disabled ? 'rgba(14,18,32,0.5)' : focused ? PALETTE.panelHi : PALETTE.panel,
        borderColor: disabled ? PALETTE.border : focused ? PALETTE.borderHot : PALETTE.border,
        borderWidth: focused ? 2 : 1.5,
        radius: o.radius === undefined ? 10 : o.radius,
      });
      if (focused && !disabled) {
        r.drawRoundRect(x, y, w, h, o.radius === undefined ? 10 : o.radius, PALETTE.accent, 0.10 + pulse * 0.08);
      }
    ...
    if (label && !o.invisible) {
      this.text(label, o.textAlign === 'left' ? x + 14 : x + w / 2, y + h / 2, {
        size: o.size || 16,
    ...
    if (o.tooltip && focused) this._tooltip = { text: o.tooltip, x, y: y + h + 8 };

    if (disabled) return false;
    const clicked = (hover && input.mouseClicked) || this.activated(idx);
    if (clicked) audio.play('uiConfirm');
    return clicked;
  }
  ...
  list(id, x, y, w, h, items, rowH, renderRow, state) {
    const r = this.r;
    const visible = Math.floor(h / rowH);
    state.scroll = clamp(state.scroll || 0, 0, Math.max(0, items.length - visible));
    if (input.wheel) state.scroll = clamp(state.scroll + input.wheel, 0, Math.max(0, items.length - visible));
    ...
    for (let i = 0; i < visible && i + state.scroll < items.length; i++) {
      ...
      const mx = input.mouseX / (r.dpr || 1), my = input.mouseY / (r.dpr || 1);
      const hover = mx >= x && mx <= x + w && my >= ry && my <= ry + rowH;
      if (hover && input.mouseClicked) { activated = idx; audio.play('uiConfirm'); }
      if (hover) this.focus = idx2;
      ...
      renderRow(items[idx], x, ry, w, rowH - 4, focused, idx);
    }
    r.unclip();
    // scrollbar
    if (items.length > visible) {
      const t = state.scroll / Math.max(1, items.length - visible);
      const bh = Math.max(24, h * (visible / items.length));
      r.drawRect(x + w - 4, y + (h - bh) * t, 3, bh, PALETTE.textFaint, 0.6);
    }
    return activated;
  }
  ...
  backButton(x, y) {
    const hit = this.button('__back', x, y, 92, 34, '‹ BACK', { size: 14 });
  ...
  _drawTooltip() {
    ...
    const r = this.r;
    const lines = wrapText(r, t.text, 320, 13);
    const h = lines.length * 17 + 16;
    const x = clamp(t.x, 8, r.w - 340);
    this.panel(x, t.y, 330, h, { color: 'rgba(6,8,16,0.96)', radius: 8 });
    for (let i = 0; i < lines.length; i++) {
      this.text(lines[i], x + 12, t.y + 16 + i * 17, { size: 13, color: PALETTE.textDim });
    }
  }
}
